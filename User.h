...
class User
{
 private:
  ...
  rank_map _rank;
  std::shared_ptr<RecommendationSystem> _rec_ptr;
public:
	/**
	 * Constructor for the class
	 */
	User(const std::string& username,const rank_map& rank,
         std::shared_ptr<RecommendationSystem> rec_ptr);
	/**
	 ...
	 */
	double get_prediction_score_for_movie(const std::string& name,  int
    year, int k) const;
